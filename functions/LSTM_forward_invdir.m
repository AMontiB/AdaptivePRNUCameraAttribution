...
function [W_transf,K_transf,num_CPCE,CPCE_inv,CPCE_dir,alpha,error,idx_partial, varW_v, varK_v, arr_ind_W, arr_ind_K] = LSTM_forward_invdir(Noise,Fingerprint,r,theta,R,xi,yi,center,ut,vt,imageSize,W_transf,num_CPCE,K_transf,k0,alpha_k0,...
    Ak,annuli_radii,fing,W,step,delta,lenk,lenf,...
...
for m=index:lenk
    ...
    [alpha_k,pos,fl,~,~,~]=prediction_check(Ak,alpha(1,m),step,ro,ri,Noise,Fingerprint,r,theta,R,xi,yi,center,ut,vt,imageSize, transf_idx);
    W_transf{m} = PRNU_transformer(Noise, alpha_k,annuli_radii(2,index),...
    ...
    K_transf{index} = fingerprint_transformator(Fingerprint, alpha_k,annuli_radii(2,index),...
        annuli_radii(1,index),r, theta,R,xi,yi,center,ut,vt,transf_idx);
    K_arr = reshape(K_transf{index},[1 imageSize(1)*imageSize(2)]);
    ind = find(K_arr<20);
    ...
    if (sum(num_CPCE(1,:))^2/denom_inv > tau_c) || (sum(num_CPCE(2,:))^2/denom_dir > tau_c)
       return 
    end
